...
We run into a problem when we try to run Knit, if you run the RMD with Run All the code works fine, but if you convert to HTML throw Knit NaïveBayes throws and error and the execution stops, the error is the following :

Error in `` `*tmp*`[[i]] ``:
! subíndice fuera de  los límites

We can't find any solution so this question won't have a HTML
